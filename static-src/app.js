...
function showConfirm(payload) {
  ...
  [['姓名', payload.name], ['手機', payload.phone || '未填寫'], ['身分證', payload.idCard || '未填寫'], ['出生年月日（民國）', payload.birthDate ? formatRocDate(payload.birthDate) : '未填寫'], ['報名對象', relationLabel(payload.relationType)], ['會員本人', payload.guardianName || '—'], ['參加選項', labels]]
...
async function submitRegistration(payload) {
  ...
  let member = null;
  const relationType = payload.relationType || 'self';
  ...
  if (relationType === 'self') {
    ...
    const guardianCandidates = members.filter((m) => m.name && normalizeKey(m.name) === normalizeKey(payload.guardianName));
    if (state.mode === 'roster') {
      if (guardianCandidates.length === 0) return showError('找不到會員本人，請確認姓名');
    } else {
      const selfKeys = new Set(state.registrations.filter((r) => !r.relationType || r.relationType === 'self').map((r) => normalizeKey(r.memberNo)));
      const registeredCandidates = guardianCandidates.filter((m) => selfKeys.has(normalizeKey(m.memberNo)));
      if (registeredCandidates.length === 0) return showError('會員本人尚未完成報名，請先由會員本人報名');
    ...
    if (guardianCandidates.length === 1) {
      ...
      const phoneMatches = guardianCandidates.filter((m) => m.phone && normalizePhone(m.phone) === normalizePhone(payload.guardianPhone));
      if (phoneMatches.length === 1) guardian = phoneMatches[0];
      else return showError('名冊中有同名會員，請輸入正確的會員本人手機號碼');
    }
    ...
    if (!guardianRegistered) return showError('會員本人尚未完成報名，請先由會員本人報名');

    member = members.find(
      (m) => m.memberNo.startsWith('D-') && normalizeKey(m.name) === normalizeKey(payload.name) && (!payload.phone || (m.phone && normalizePhone(m.phone) === normalizePhone(payload.phone)))
    );
    if (member) {
      Object.assign(member, { name: payload.name, phone: payload.phone, email: payload.email, idCard: payload.idCard, birthDate: payload.birthDate });
    } else {
      member = { memberNo: `D-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, name: payload.name, phone: payload.phone, email: payload.email, idCard: payload.idCard, birthDate: payload.birthDate, department: '' };
      members.push(member);
    }
    payload._guardian = guardian;
  }
  ...
  if (existingReg) {
    ...
    existingReg.relationType = relationType;
    existingReg.guardianMemberNo = relationType === 'self' ? '' : payload._guardian.memberNo;
    existingReg.guardianName = relationType === 'self' ? '' : payload._guardian.name;
    ...
    state.registrations.push({
      ...
      relationType,
      guardianMemberNo: relationType === 'self' ? '' : payload._guardian.memberNo,
      guardianName: relationType === 'self' ? '' : payload._guardian.name,
      submittedAt: now
    });
  }
  delete payload._guardian;
  saveState();
...
function showResult(title, payload, submittedAt) {
  ...
  box.append(
    ...
    el('p', {}, `報名對象：${relationLabel(payload.relationType)}${payload.guardianName ? `｜會員本人：${payload.guardianName}` : ''}`),
    el('p', {}, `出生年月日：${payload.birthDate ? formatRocDate(payload.birthDate) : '未填寫'}`),
...
async function handleSubmit(event) {
  ...
  if (birthText && !birthDate) return showError('出生年月日格式不正確，請輸入民國年，例如 113/05/20');
  if (relationType !== 'self' && !guardianName) return showError('親屬或未滿12歲報名需填寫會員本人姓名');
  const payload = { name, phone, email, idCard, birthDate, relationType, guardianName, guardianPhone, options };
  if (idCard || birthDate || relationType !== 'self') showConfirm(payload);
...
function exportPeopleCsv() {
  ...
  const headers = peopleMode === 'registered' ? ['會員編號', '姓名', '部門', '手機', 'Email', '報名對象', '會員本人', '身分證', '出生年月日(民國)', '參加選項', '報名時間'] : ['會員編號', '姓名', '部門', '手機', 'Email'];
  const rows = [headers];
  list.forEach((member) => {
    if (peopleMode === 'registered') {
      ...
      rows.push([
        ...
        (reg.options || []).map((id) => optionLabel(id)).join('、'),
