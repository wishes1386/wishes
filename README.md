...
`public/報名表單-靜態版.html` 是單一 HTML 檔，包含報名頁、後台、QR Code、資料檢查與 CSV 匯入，不需要 Node 伺服器，放到 Google Drive 或任何靜態網頁空間都能直接用瀏覽器開啟。

靜態版的資料會存在開啟者自己的瀏覽器 localStorage；正式多人共用的集中式報名仍建議使用 `node server.js` 的主版本。

## 分享給會員

詳細步驟請見「分享給會員步驟.txt」。Google Drive 本身不提供 HTML 網頁直接開啟，建議把「報名表單.html」拖到 Netlify Drop 取得正式網址後，貼到後台「QR Code 網址」欄位，再將網址或 QR Code 發給會員。
